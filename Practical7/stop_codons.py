...
def write_fasta(header, seq, fh):
    fh.write(header + "\n")
    for i in range(0, len(seq), 60):
        fh.write(seq[i:i+60] + "\n")

with open("Saccharomyces_cerevisiae.R64-1-1.cdna.all.fa") as f_in, \
     open("stop_genes.fa", "w") as f_out:
    ...
    for line in f_in:
        ...
        if line.startswith(">"):
            if current_header:
                s = current_seq.replace("\n", "")
                start = s.find("ATG")
                found = []
                if start != -1:
                    for i in range(start + 3, len(s), 3):
                        c = s[i:i+3]
                        if c in stop_codons and c not in found:
                            found.append(c)
                if found:
                    gene_id = current_header.split()[0].replace(">", "")
                    new_header = f">{gene_id};{'_'.join(found)}"
                    write_fasta(new_header, s, f_out)
